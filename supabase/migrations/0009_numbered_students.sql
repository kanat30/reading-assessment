...
ALTER TABLE assessments
  ADD COLUMN IF NOT EXISTS use_numbered_students boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS expected_student_count integer CHECK (expected_student_count IS NULL OR expected_student_count > 0);

-- Add index for querying numbered student assessments
CREATE INDEX IF NOT EXISTS assessments_numbered_students_idx ON assessments(use_numbered_students) WHERE use_numbered_students = true;
