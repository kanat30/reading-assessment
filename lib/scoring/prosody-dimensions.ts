/**
 * Deterministic prosody dimensions — CLIENT-SAFE module.
 *
 * This file must stay free of lib/scoring/ai.ts (which constructs the
 * Anthropic client at module scope and throws in the browser): these helpers
 * are imported by client components (report surfaces, dashboard) as well as
 * the server scoring pipeline. The AI NAEP observation lives in prosody.ts,
 * which is server-only.
 */

import { SessionEvent, ProsodyDimensions, ProsodyDimensionValue } from "./types";
