...
- **NYC DOE AI-guidance compliance pass** — public explainability page, advisory-only framing on AI outputs, and teacher-auth gating of student audio (biometric treatment) shipped 2026-07-10; remaining: finalize the vendor disclosure one-pager (working draft = `/district` print view, 2026-07-24), data-retention/auto-deletion policy, full copy audit for placement/surveillance language — in-progress (compliance gate)
- **Seed dashboard with realistic sessions before the demo** — addresses content-density perception; a complete feature set can still feel sparse — done (`scripts/seed-demo.ts`: isolated demo tenant, 19 students / 50 sessions scored by the real engine; see STATUS.md)
