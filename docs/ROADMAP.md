...
- **Finish & commit word-level teacher overrides** — core V1 trust feature: per-word approve/reject of AI-flagged errors plus the ability to add missed words; corrections logged for manual parameter tuning (no automated retraining, per ERMA) — in-progress (uncommitted per STATUS.md)
- **Finish & commit comprehension partial-credit + regrade** — completes the end-of-passage comprehension *check* that qualifies the fluency score — in-progress (uncommitted per STATUS.md)
...
- **NYC DOE AI-guidance compliance pass** — make override more prominent, add a public explainability page, reframe copy as advisory-only, drop placement/surveillance language, treat audio as potential biometric, prep the vendor disclosure one-pager — planned (compliance gate)
